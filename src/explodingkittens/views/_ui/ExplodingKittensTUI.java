...
import explodingkittens.models._cards.actions.FavorCard;
import explodingkittens.models._cards.defuse.DefuseCard;
import explodingkittens.models._cards.explode.ExplodingKittenCard;
import explodingkittens.models._cards.std.ActionCard;
import explodingkittens.models._cards.std.BasicCard;
import explodingkittens.models._cards.std.ExplodingKittensCardBlank;
import explodingkittens._game.Game;
import explodingkittens._game.OfflineExplodingKittensGame;
...
public class ExplodingKittensTUI implements ExplodingKittensUI {
    ...
    private void gameMenu(Game game) throws NoPlayerFound {
        ...
        while (!game.hasWinner()) {
            BasicCard chosenCard = promptCard(player);
     ...
     * @return
     */
    private BasicCard promptCard(Player player) {
        gameConsole.out.println();
     ...
     */
    private void printCards(List<BasicCard> cards) {
        ...
        private void printBlankCards(List<BasicCard> cards) {
            for (int i = 0; i < cards.size(); i++) {
            if (i > 0)
                gameConsole.out.println();
            gameConsole.out.printf("(%d)\n", i + 1);
            gameConsole.out.println(drawCard(new ExplodingKittensCardBlank(), i + 1));
        }
    }
     ...
     */
    public String drawCard(BasicCard card, int cardNumber) {
        int artWidth = 30;
        StringBuilder cardArt = new StringBuilder();

        cardArt.append('+').append("-".repeat(artWidth - 2)).append('+').append('\n');
        if (!(card instanceof ExplodingKittensCardBlank)) {
    ...
    /**
     * Implements the userAccept method from the ExplodingKittensUI interface.
     *
